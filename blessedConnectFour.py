class ConnectFour:
    ...
    with term.cbreak(), term.hidden_cursor():
        ...
        start_txt = False
        start = (
            term.move_xy(term.width // 2 - 8, term.height * 3 // 4) + "Press S to start"
        )
        start_erase = (
            term.move_xy(term.width // 2 - 8, term.height * 3 // 4) + "                "
        )
    ...
    def get_nrow_ncol(self):
        ...
        size = (
            self.term.move_xy(self.term.width // 2 - 32, self.term.height // 3)
            + "Please enter the size of the board "
            + "in the format HEIGHT x WIDTH.\n"
        )
        print(size, end="", flush=True)
        size_buffer = (
            self.term.move_y(self.term.height // 2) + f"\n{' '*(self.term.width//2-2)}"
        )
        ...
        while True:
            try:
                ...
                error = (
                    self.term.move_xy(self.term.width // 2 - 18, self.term.height // 3)
            ...
            try:
                ...
                error = (
                    self.term.move_xy(self.term.width // 2 - 22, self.term.height // 3)
            ...
            if num_row < self.min_nRow_nCol or num_col < self.min_nRow_nCol:
                error = (
                    self.term.move_xy(self.term.width // 2 - 20, self.term.height // 3)
                ...
                error = (
                    self.term.move_xy(self.term.width // 2 - 15, self.term.height // 3)
    ...
    def start(self):
        ...
        col_head = (
            self.term.move_xy(
                self.term.width // 2 - (self.num_col * 4 + 1) // 2,
                self.term.height // 4,
            )
            + col_head_txt
        )
        ...
        prompt1_p1 = (
            self.term.move_xy(
                self.term.width // 2 - 8, self.term.height // 4 + self.num_row + 5
            )
        ...
        prompt1_draw = (
            self.term.move_xy(
                self.term.width // 2 - 6, self.term.height // 4 + self.num_row + 5
            )
            + "Game is over!"
        )
        prompt1_erase = (
        ...
        prompt2 = (
            self.term.move_xy(
                self.term.width // 2 - 18, self.term.height // 4 + self.num_row + 7
            )
            + "Choose a column by typing its letter."
        )
        prompt2_error = (
            self.term.move_xy(
                self.term.width // 2 - 21, self.term.height // 4 + self.num_row + 7
            )
            + f"Invalid column. Choose a column from A to {last_col}"
        )
        prompt2_full = (
            self.term.move_xy(
                self.term.width // 2 - 23, self.term.height // 4 + self.num_row + 7
            )
    ...
    def check_win(self, cur_player, column, row):
        ...
        count = 0
        diag = self.np.diag(
            self.np.fliplr(self.matrix), (self.num_col - 1 - column) - row
        )
        for i in diag:
            if i == cur_player:
                count += 1
